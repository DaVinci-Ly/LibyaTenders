...
# التحقق مما إذا كان المنفذ 7845 مشغولاً بالفعل وإغلاقه لتفادي مشاكل التشغيل المتكرر
PID=$(lsof -i :7845 -sTCP:LISTEN -t)
if [ ! -z "$PID" ]; then
    echo -e "${YELLOW}⚠️ النظام يعمل بالفعل في الخلفية (PID: $PID).${NC}"
    echo -e "${YELLOW}🔄 جاري إيقاف النسخة السابقة وبدء تشغيل نسخة جديدة...${NC}"
    kill -9 $PID 2>/dev/null
    sleep 1
    echo -e "${GREEN}✅ تم إيقاف النسخة السابقة بنجاح.${NC}"
    echo ""
fi

echo -e "${GREEN}🔄 جاري تشغيل خادم النظام وفتح المتصفح...${NC}"
echo ""
